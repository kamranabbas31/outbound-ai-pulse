...
const Sidebar: FC<SidebarProps> = ({
  ...
  return <div className="hidden md:flex flex-col h-screen w-56 border-r bg-white">
      ...
      <div className="flex flex-col flex-1 p-4">
        ...
        <nav className="flex flex-col space-y-1">
          {menuItems.map(item => <Link key={item.path} to={item.path} className={`flex items-center h-10 px-4 py-2 text-sm rounded-md ${currentPath === item.path ? "bg-muted text-primary font-medium" : "text-muted-foreground hover:bg-muted"}`}>
